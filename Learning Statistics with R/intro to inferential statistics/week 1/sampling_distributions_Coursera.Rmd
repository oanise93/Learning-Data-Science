...
samp1 %>%
  summarize(x_bar = mean(area), samp1_med = median(area), s = sd(area),
            samp1_iqr = IQR(area), samp1_min = min(area), samp1_max = max(area), 
            samp1_q1 = quantile(area, 0.25), samp1_q3 = quantile(area, 0.25))
```
...
sample_means50 %>%
  summarize(x_bar_mean = mean(x_bar), se = sd(x_bar))
...
```{r price-sampling}
# type your code for this Exercise here, and Run Document
sample_means_prices_50 <- ames %>% rep_sample_n(size=50, reps=5000, replace = TRUE) %>% summarise(x_bar = mean(price))

ggplot(data=sample_means_prices_50, aes(x=x_bar)) + geom_histogram(binwidth = 500)

sample_means_prices_50 %>%
  summarize(mean_price = mean(x_bar))
```
...
sample_means150 <- ames %>% rep_sample_n(size=150, replace=TRUE, reps=5000) %>% summarize(x_bar = mean(price))
ggplot(data=sample_means150, aes(x=x_bar)) + geom_histogram(binwidth=500)
...
# type your code for this Exercise here, and Run Document
mean_price <- ames %>% sample_n(size=15) %>% summarize(x_bar = mean(price))
...
# type your code for this Exercise here, and Run Document
sample_means_15 <- ames %>% rep_sample_n(size=15, replace=TRUE, reps=2000) %>% summarize(x_bar = mean(price))
ggplot(data=sample_means_15, aes(x=x_bar)) + geom_histogram(binwidth = 1000)
```
