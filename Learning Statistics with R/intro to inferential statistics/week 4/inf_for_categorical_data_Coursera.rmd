...
<br> Conditions:
<ul>
  <li>The sample observations are independent - We can safely assume the observations are independent with each individual only being interviewed once./li>
  <li>At least two successes (atheist) and 10 failures - checks out because we approximately 950 non-atheists 50 atheists</li>
<ul>
</div>
...
<br> <br> The margin error is the amount that we would expect the result vary by just due to chance.
</div>
...
p = sum(us12$response == "atheist") / length(us12$response)
...
nigeria12 <- atheism %>%
  filter(nationality == "Nigeria" , atheism$year == "2012")

#checking for conditions
prop.table(table(nigeria12$response))
prop.table(table(canada12$response))

n_nigeria = length(nigeria12$response)
n_canda = length(canada12$response)


# constructing confidence intervals
inference(y = response, data = canada12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
inference(y = response, data = nigeria12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
...
# hypthoses
# Null hypothesis: There is no difference between the proportion of people from Spani who are atheist in 2005 and 2012. If there is any difference, it is just due to chance
# Alternative hypothesis: There is a difference between the proportion of people from Spain.
...
n_spain05 = length(spain05$response)
n_spain12 = length(spain12$response)
p_atheist_spain05 = sum(spain05$response == "atheist") / n_spain05
p_atheist_spain12 = sum(spain12$response == "atheist") / n_spain12

(n_spain05 * p_atheist_spain05) > 10
(n_spain12 * p_atheist_spain12) > 10

(n_spain05 * (1- p_atheist_spain05)) > 10
(n_spain12 * (1 - p_atheist_spain12)) > 10

spain <- atheism %>%
  filter(nationality == "Spain")

inference(y = response, x = year, data = spain, type = "ht", success="atheist", statistic = "proportion",  method = "theoretical", alternative = "twosided")
...
inference(y = response, x = year, data = us05_12, type = "ht", success="atheist", statistic = "proportion",  method = "theoretical", alternative = "twosided")
```
